...
class UpdateItemRequestProvisionServiceImplTest {
    ...
    void setUp() {
        service = new UpdateItemRequestProvisionServiceImpl(
                mapper, itemRepository, requestRepository, provisionRepository, statusRepository, eventPublisher
        );
        request = new Request();
        request.setId(10L);
    ...
    void shouldNotSaveOrPublishWhenValidationFailsBeforeSave() {
        ...
        when(provisionRepository.findById(20L)).thenReturn(Optional.empty());

        assertThrows(ProvisionNotFoundException.class, () -> service.updateItem(99L, requestDto(2L)));
    ...
    private void arrangeSuccessfulUpdate(Status status) {
        when(itemRepository.findById(99L)).thenReturn(Optional.of(item));
        when(requestRepository.findById(10L)).thenReturn(Optional.of(request));
        when(provisionRepository.findById(20L)).thenReturn(Optional.of(provision));
